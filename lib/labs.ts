...
export function scoreTransaction(input: Omit<Transaction, "id" | "fraud" | "logisticScore" | "forestScore" | "score" | "risk">) {
  ...
  const logistic = 4 + (input.amount > 25000 ? 20 : input.amount > 10000 ? 9 : 0) + (night ? 17 : 0) + (unknown ? 22 : 0) + (input.newDevice ? 25 : 0) + Math.min(18, failedAttempts * 6) + (input.senderBank !== input.receiverBank ? 4 : 0);
  const forest = 3 + (input.amount > 50000 ? 28 : input.amount > 15000 ? 13 : 0) + (night ? 20 : 0) + (unknown ? 26 : 0) + (input.newDevice ? 21 : 0) + Math.min(20, failedAttempts * 7) + (input.type === "P2P" ? 4 : 0);
...
export function generateTransactions(count = 1000, fraudPercent = 10, seed = 42): Transaction[] {
  ...
  return Array.from({ length: Math.min(50000, Math.max(100, count)) }, (_, index) => {
    ...
    const hour = normalFraud ? 6 + Math.floor(random() * 17) : suspiciousLegit ? (random() < 0.5 ? [0, 1, 2, 3, 4, 22, 23][Math.floor(random() * 7)] : 6 + Math.floor(random() * 17)) : fraud ? [0, 1, 2, 3, 4, 22, 23][Math.floor(random() * 7)] : 6 + Math.floor(random() * 17);
