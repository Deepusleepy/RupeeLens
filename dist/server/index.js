globalThis.__VINEXT_LAZY_CHUNKS__ = ["assets/rupee-lens-tD9QMrQC.js","assets/layout-segment-context-BTGCkz4X.js"];
...
function getDraftSecret() {
	return "e4db4cf4-910a-457e-a6d0-dddd7711bc1b";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "9c792b20-9427-4109-859f-13d8c0c07c0e";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "9c792b20-9427-4109-859f-13d8c0c07c0e") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "9c792b20-9427-4109-859f-13d8c0c07c0e",
		rootBoundaryId
	});
}
/**
...
var RupeeLens = /* #__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'RupeeLens' is called on server");
}, "96d64da4a948", "RupeeLens");
//#endregion
//#region app/page.tsx
var page_exports = /* @__PURE__ */ __exportAll({ default: () => Home });
function Home() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(RupeeLens, {});
}
...
//#endregion
//#region app/api/events/route.ts
...
async function GET(request) {
	...
	if (!(/* @__PURE__ */ new Set([
		...
		"review",
		"held"
	])).has(status)) return Response.json({ error: "Unsupported status filter" }, { status: 400 });
...
async function POST(request) {
	...
	if (!inRange(input.amount, 1, 1e6) || !inRange(input.hour, 0, 23) || !inRange(input.deviceAgeDays, 0, 3650) || !inRange(input.failedAttempts, 0, 20) || !inRange(input.locationVelocityKmH, 0, 2e3) || typeof input.newBeneficiary !== "boolean" || typeof input.vpaMismatch !== "boolean") return Response.json({ error: "Scenario fields are missing or outside accepted ranges" }, { status: 400 });
	const contributions = [];
	if (input.amount > 25e3) contributions.push({
		signal: "High value",
		points: 18,
		detail: "Amount is above ₹25,000"
	});
	else if (input.amount > 1e4) contributions.push({
		signal: "Elevated value",
		points: 9,
		detail: "Amount is above ₹10,000"
	});
	if (input.hour < 5) contributions.push({
		signal: "Low-activity hour",
		points: 14,
		detail: "Payment falls between midnight and 05:00"
	});
	if (input.location === "Unknown" || input.location === "Foreign") contributions.push({
		signal: "Untrusted location",
		points: 20,
		detail: `${input.location} is treated as a high-review origin in this demonstration`
	});
	if (input.deviceAgeDays < 3) contributions.push({
		signal: "New device",
		points: 24,
		detail: "Device was enrolled less than three days ago"
	});
	if (input.failedAttempts >= 2) contributions.push({
		signal: "Repeated attempts",
		points: 16,
		detail: "Two or more failures preceded the payment"
	});
	if (input.newBeneficiary) contributions.push({
		signal: "New beneficiary",
		points: 13,
		detail: "No prior payment relationship is present"
	});
	if (input.vpaMismatch) contributions.push({
		signal: "Identity mismatch",
		points: 22,
		detail: "VPA and account identity fields disagree"
	});
	if (input.locationVelocityKmH > 500) contributions.push({
		signal: "Impossible travel",
		points: 28,
		detail: "Location change exceeds plausible physical travel"
	});
	else if (input.locationVelocityKmH > 180) contributions.push({
		signal: "Location velocity",
		points: 12,
		detail: "Location changed unusually quickly"
	});
	...
	return Response.json({
		...
		contributions: contributions.length ? contributions : [{
			signal: "Baseline",
			points: 6,
			detail: "No elevated demo rule was observed"
		}],
...
var routes = [
	...
	},
	{
		...
		ids: {
			...
			"page": null,
			"routeHandler": "route-handler:/api/events",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/risk",
			"page": null,
			"routeHandler": "route-handler:/api/risk",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
...
var __configHeaders = [];
var __publicFiles = /* @__PURE__ */ new Set(["/favicon.svg"]);
